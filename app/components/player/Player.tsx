...
const Player: React.FC<PlayerProps> = ({
  ...
  return (
    <div className={cn("w-[9.5rem] flex flex-col items-center justify-center", {
      "w-[6.5rem]" : isGameOver,
      "bg-button-dark-blue-text py-[0.875rem] rounded-[0.625rem] shadow-[0px_4px_11.3px_0px_rgba(0,0,0,0.25)]" : isGameOver && isWinner
    },className)}>
      <div
      ...
      >
        ...
        <div className="flex gap-2 items-center">
          {(presenceStatus && !isGameOver) &&
            ...
            <div>
              <p className={`font-normal text-lg leading-[1.5rem] text-center ${(isGameOver || isWinner) && "text-[1.25rem] text-button-dark-blue"}`}>{username}</p>
              {(isWinner && prize) && (
                <>
                  <p className="text-[1.25rem] text-button-dark-blue leading-[1.5rem] text-center">Wins!</p>
        ...
        {/* Progress bar indicating the state of the correctly picked cards out of total cards available*/}
