...
const GameCustomizeNavBar = ({handleShareGameLink, handleGoBack, handleHowToPlayClick, handleShowMenu}: PageProps) => {
  return (
    <Navbar className='flex justify-between'>
      ...
      <div className='md:flex hidden gap-6 items-center'>
        ...
        <div className='md:flex hidden items-center flex-row-reverse gap-4'>
          <button onClick={() => handleHowToPlayClick} className='flex items-center border rounded-[8px] gap-[.5em] border-textColor-main text-textColor-main justify-center py-2 px-4 outline-1'>
            <span>?</span>
          </button>
          <button className='flex items-center gap-1 bg-[#00658B] shadow-custom-inset rounded-[8px] gap[.5em] text-[#D5F3FF] justify-center py-[10px] px-4 outline-1'>
            <span><UserRound /></span>
            <span>Friends</span>
