...
const GameCustomizeNavBar = ({ handleShareGameLink, handleGoBack }: PageProps) => {
      ...
      <Navbar className="sticky left-0 top-0 z-[999]">
        <div className="container flex justify-between">
          ...
          <div className="md:flex hidden items-center gap-4 text-primary-700">
            <button onClick={() => router.back()} className="flex items-center border rounded-[8px] gap-[.5em] border-primary-600 justify-center py-2 px-4 outline-1 hover:bg-primary-700 hover:text-white">
              <span><ArrowLeft /></span>
              <span className="text-textColor-main text-sm">Back</span>
            </button>
          </div>
          <div className="md:flex hidden gap-6 items-center text-sm">
            <button onClick={() => handleShareGameLink()} className="md:flex hidden items-center shadow-custom-inset bg-primary-yellow-400 rounded-[8px] gap-2 text-primary-yellow-900 justify-center py-3 px-4 hover:opacity-85">
              <span>Share Game Invite</span>
              <span><Link size={"14px"} /></span>
            </button>
            <div className="md:flex hidden items-center flex-row-reverse gap-4">
              <button onClick={() => handleHowToPlayClick} className="flex items-center border rounded-[8px] gap-[.5em] border-primary-700 text-primary-700 justify-center py-2 px-4 outline-1 hover:bg-primary-700 hover:text-white">
                <span>?</span>
              </button>
              <div className="flex flex-col gap-1 relative">
                <button onClick={handleDisplayFriends} className="flex relative items-center gap-1 bg-primary-700 shadow-custom-inset rounded-[8px] gap[.5em] text-primary-100 justify-center py-[10px] px-4 outline-1 hover:bg-transparent hover:text-primary-700">
                  <span><UserRound className="w-5" /></span>
                  <span>Frieda</span>
